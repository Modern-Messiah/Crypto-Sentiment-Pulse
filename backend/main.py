...
from database import init_db, SessionLocal, PriceHistory
...
async def get_history(symbol: str, period: str = "15m", limit: int = 100):
    ...
    if period == "15m" and bs_module.binance_stream:
        ...
        if len(mem_history) >= limit:
            return {"symbol": symbol, "source": "memory", "period": period, "history": mem_history[-limit:]}
    ...
    try:
        ...
        return {"symbol": symbol, "source": "database", "period": period, "history": formatted_history}
...
async def health_check():
    ...
    connected = bs_module.binance_stream is not None and len(bs_module.binance_stream.get_prices()) > 0
    return {
